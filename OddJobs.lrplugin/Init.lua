...
do
    ...
    if not LrFileUtils.isWritable( _PLUGIN.path ) then -- typically Windows "Program Files" dir or Mac's "Applications" folder.
        error( "Plugin directory must be writable, '".._PLUGIN.path.."' is not. Remedy by re-locating to writable user folder like \"Documents\"." )
    end
...
_G.OddJobsObjectFactory = require( 'OddJobsObjectFactory' )    -- class of object used to create objects of classes not mandated by the framework.
_G.objectFactory = OddJobsObjectFactory:new()               -- object used to create objects of classes not mandated by the framework.
...
_G.OddJobsManager = require( "OddJobsManager" )
--_G.OddJobsBackground = require( "OddJobsBackground" ) - obsolete.
_G.OddJobs = require( "OddJobs" )



--   I N I T I A T E   A S Y N C H R O N O U S   I N I T   A N D   B A C K G R O U N D   T A S K
OddJobsManager.initPrefs() -- remember to include all dependent plugin prefs (plugin generator is not helping in this regard, yet)
oddJobs = OddJobs:new()
app:initDone() -- synchronous init done.
-- Consider async init/background task.
if app:getPref( 'autoStartMaintainPhotoSelections' ) then
    oddJobs:maintainPhotoSelections()
end
